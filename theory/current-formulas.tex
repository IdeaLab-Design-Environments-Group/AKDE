...
validation gates, and the currently selected minor-cut formula. Where an input
is carried by the product but not consumed by the geometry derivation, that is
stated explicitly instead of back-filling aspirational math.
...
In the current implementation, \(L_o\) affects pattern construction and export
layout, but it does not enter the scalar geometry derivation in
\texttt{kirigami/model/geometry.ts}.
...
The implementation exposes the derived values
\[
\texttt{moleculeEndLeg} = D,
\qquad
\texttt{minorCutLength} = \ell.
\]
