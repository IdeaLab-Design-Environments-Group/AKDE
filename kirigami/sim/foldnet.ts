...
export function buildFoldNet(state: KirigamiState): FoldNet {
  ...
  for (let k = 0; k < N; k++) {
    ...
    // Lateral-face triangle: tip + the two outer base corners.
    addTri(tip[k], polyOuterL[k], polyOuterR[k]);
    // Both slants are mountain creases (shared with the neighbouring molecule halves).
    ...
    // Left half (innerL, innerMid, foldPt, outerR1, tip[k]) — outer wedge above foldPt removed.
    addTri(innerMid, foldPt, outerR1);
    addTri(innerMid, outerR1, tip[k]);
    addTri(innerMid, tip[k], innerL);
    // Right half (innerR, innerMid, foldPt, outerL2, tip[k+1]).
